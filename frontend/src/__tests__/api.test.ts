...
describe("demo api behavior", () => {
  ...
  it("returns polling locations for an address", async () => {
    const { default: api } = await import("../services/api");
    const response = await api.post("/polling/search", { address: "123 Main" });
  ...
  it("rejects unknown demo endpoints", async () => {
    const { default: api } = await import("../services/api");
    await expect(api.get("/unknown")).rejects.toHaveProperty("response.status", 404);
  });

  it("responds with guidance to ID questions", async () => {
    const { default: api } = await import("../services/api");
    const response = await api.post("/chat", { message: "Do I need an id?" });
